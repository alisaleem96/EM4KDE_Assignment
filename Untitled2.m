...
for iter = 1:max_iter
    ...
    for fold = 1:nfolds
        ...
        log_likelihood(fold,iter) = sum(log(sum(tmp_var,2)),1);
    end
    % Extract the maximum likelihood sigma of the 10 folds
    [maxLL, maxIdx] = max(log_likelihood(:,iter));
    LL(iter) = maxLL;
    
    % Put the MLE of sigma
    Sigma = (1/N) * sum(Sigmas,3);
    %log_likelihood = zeros(nfolds,1);
    % End...
    if iter > 5
